    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            [[TZImageManager manager] getVideoWithAsset:model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
                ...
                NSString *str = [NSString stringWithFormat:@"%@",((AVURLAsset *)playerItem.asset).URL];
                str = [str stringByReplacingOccurrencesOfString:@"file://" withString:@""];
                
                
                NSInteger   fileSize = [[NSFileManager defaultManager] attributesOfItemAtPath:str error:nil].fileSize;
                float memorySize = fileSize / 1024 / 1024;
//                NSLog(@"%f",memorySize);
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if (memorySize <= 50) {
                        ...
                        NSLog(@"视频大于50M");
                        TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
                        [imagePickerVc showAlertWithTitle:@"视频大于50M"];
                    }
                });
